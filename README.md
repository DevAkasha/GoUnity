...
**GoUnity**는 범용성을 중시하여 튼튼한 토대를 만들고자 노력한 3D 메타버스 프로젝트입니다. 복잡한 캐릭터 시스템을 다루며, 제네릭을 활용한 확장 가능한 아키텍처에 중점을 두어 개발되었습니다.
...
### 설치 방법
1. 프로젝트를 Unity에서 열기
2. Package Manager에서 필요한 패키지 설치
3. Scene/Main 씬 로드
4. 플레이 버튼 클릭
...
### 새로운 상호작용 오브젝트 추가
1. `InteractableStuff` 또는 `InteractableItem` 상속
2. `ApplyPlayer()` 메서드 구현
3. ScriptableObject로 데이터 생성
4. 예시:
```csharp
...
### 새로운 UI 창 추가
1. `UIBase` 상속 클래스 생성
2. `UIManager.Show<T>()` 사용
3. Resources 폴더에 프리팹 배치

### 새로운 매니저 추가
1. `Manager<T>` 상속
2. 필요한 로직 구현
3. 다른 매니저에서 `Instance` 접근
...
### 개발자
- **허민영** - 메인 개발자
- **티스토리**: [개발 블로그](https://devakasha.tistory.com/)

### 크레딧
- **Unity-Chan**: Unity Technologies Japan
- **IronCow** : 소재철 개발자님 프레임워크
